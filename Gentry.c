...
void Extraction(element_t SP1, element_t SP2, element_t PID1i, element_t PID2i, element_t y, element_t z, element_t P, element_t Vi, pairing_t pairing) {
...
int aggregate_verify(
    ...
    element_t  P_agg, H_agg, temp_G1, P1[q], P2[q], P3[q];
...
{   Vehicle V[40];
    ...
    for(int i=0; i<40; i++){
    Extraction(V[i].SP1, V[i].SP2, V[i].PID1i, V[i].PID2i, y, z, P, Vi[i], pairing); 
    }
  ...
  elapsed12 = ((end12.tv_sec - start12.tv_sec) + (end12.tv_nsec - start12.tv_nsec) / 1e9)/40;   
    ...
    for(int i=0; i<40; i++){
       sign(si[i], ti[i], V[i].PID1i, V[i].PID2i, V[i].SP1, V[i].SP2, P, msgs[i], t, pairing);
    } 
   ...
   printf("Execution time of an extraction algorithm from TA's side: %f seconds\n", elapsed1); 
    ...
    int q=j;
    for(int k=0; k<40; k++){
    int result=aggregate_verify(pairing, V, si[0], ti[0], msgs,   t, P, Y, j);
    }
